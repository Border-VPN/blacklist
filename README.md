...
2. Объединяет его с текущим `blacklist.txt`, нормализует записи и убирает дубликаты (скрипт: `scripts/merge_blacklist.py`).
   - **Важно:** inline‑комментарии (все после `#` на строке) сохраняются. Для дубликатов сохраняется комментарий из первой встреченной записи (обычно из локального файла, затем из upstream).
